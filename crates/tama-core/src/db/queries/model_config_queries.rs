...
pub fn get_model_config(conn: &Connection, id: i64) -> Result<Option<ModelConfigRecord>> {
    ...
    let mut rows = stmt.query_map([id], |row| {
        Ok(ModelConfigRecord {
            ...
            selected_mtp_model: row.get(8)?,
            context_length: row.get(9)?,
            num_parallel: row.get(10)?,
            kv_unified: row.get::<_, i32>(11)? != 0,
            gpu_layers: row.get(12)?,
            cache_type_k: row.get(13)?,
            cache_type_v: row.get(14)?,
            port: row.get(15)?,
            args: row.get(16)?,
            sampling: row.get(17)?,
            modalities: row.get(18)?,
            profile: row.get(19)?,
            api_name: row.get(20)?,
            health_check: row.get(21)?,
            hf_format: row.get(22)?,
            hf_base_model: row.get(23)?,
            hf_pipeline_tag: row.get(24)?,
            hf_total_params: row.get(25)?,
            hf_active_params: row.get(26)?,
            hf_architecture_type: row.get(27)?,
            hf_context_length: row.get(28)?,
            hf_num_layers: row.get(29)?,
            hf_last_modified: row.get(30)?,
            spec_decoding: row.get(31)?,
