//! In-crate (in-file `#[cfg(test)]`) Postgres test harness.
//!
//! In-file unit tests cannot reach the integration-test harness at
//! `crates/tama-core/tests/common/mod.rs` (it references `tama_core::` by
//! name, which is not resolvable from inside the crate). This module is the
//! same harness with crate-internal paths: one `postgres:16` instance shared
//! by every test binary on the machine (via `TAMA_TEST_PG_DSN`, or the
//! `tama-test-pg` container on 127.0.0.1:`TAMA_TEST_PG_PORT`, default 5433 —
//! clean up with `make docker-clean`), plus per-test isolated schemas with
//! the squashed migrations applied.
//!
...
pub async fn with_schema() -> SchemaGuard {
    ...
    let schema = format!("tama_infile_{pid}_{n:04}");
    ...
    crate::db::postgres::run_migrations(&pool)
        .await
