...
//! same harness with crate-internal paths: a process-global `postgres:16`
//! container plus per-test isolated schemas with the squashed migrations
//! applied.
//!
...
pub async fn with_schema() -> SchemaGuard {
    ...
    let schema = format!("tama_infile_{n:04}");
