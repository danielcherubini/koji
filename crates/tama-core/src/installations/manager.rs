...
impl InstallationManager {
    ...
    pub fn open(config_dir: &std::path::Path) -> Result<Self> {
        ...
        crate::installations::migration::migrate_legacy_backends(&open_result.conn, &backends_dir)
    ...
    pub fn get_config(
        ...
        crate::db::queries::get_installation_config(&self.conn, name, gpu_variant)
    }
    ...
    pub fn list_configs(&self) -> Result<Vec<crate::db::queries::InstallationConfigRecord>> {
        crate::db::queries::list_installation_configs(&self.conn)
    }
    ...
    pub fn available_installations(&self) -> Result<Vec<InstallationOption>> {
        let active = crate::db::queries::list_active_installations(&self.conn)?;
    ...
    /// Delegates to `insert_installation` which handles INSERT OR REPLACE
    /// and deactivates other versions of the same (name, gpu_variant).
    pub fn add_installation(&self, info: &InstallationInfo) -> Result<()> {
        let record = Self::info_to_record(info)?;
        crate::db::queries::insert_installation(&self.conn, &record)
            .with_context(|| format!("Failed to insert backend '{}'", info.name))
    }

    /// Get the active installation for a name + variant.
    pub fn get_active(&self, name: &str, gpu_variant: &str) -> Result<Option<InstallationInfo>> {
        let record = crate::db::queries::get_active_installation(&self.conn, name, gpu_variant)?;
        match record {
    ...
    pub fn list_active(&self) -> Result<Vec<InstallationInfo>> {
        let records = crate::db::queries::list_active_installations(&self.conn)?;
    ...
    pub fn rename(&self, old_name: &str, new_name: &str) -> Result<bool> {
        crate::db::queries::rename_installation(&self.conn, old_name, new_name)
    }
    ...
    pub fn list_versions(
        ...
        let records =
            crate::db::queries::list_installation_versions(&self.conn, name, gpu_variant)?;
        if records.is_empty() {
            Ok(None)
        } else {
            Ok(Some(
    ...
    pub fn get_by_version(
        ...
        let record = crate::db::queries::get_installation_by_version(
            &self.conn,
            name,
            gpu_variant,
            version,
    ...
    pub fn activate(&self, name: &str, gpu_variant: &str, version: &str) -> Result<bool> {
        crate::db::queries::activate_installation_version(&self.conn, name, gpu_variant, version)
    }
    ...
    pub fn remove_version(&self, name: &str, gpu_variant: &str, version: &str) -> Result<()> {
        ...
        let existing = crate::db::queries::get_installation_by_version(
            &self.conn,
            name,
            gpu_variant,
            version,
        ...
        crate::db::queries::delete_installation(&self.conn, name, gpu_variant, version)?;

        // If we deleted the active version, activate the newest remaining one
        if was_active {
            let remaining = crate::db::queries::list_installation_versions(
                &self.conn,
                name,
                Some(gpu_variant),
            )?;
    ...
    pub fn delete_all_versions(&self, name: &str, gpu_variant: Option<&str>) -> Result<()> {
        crate::db::queries::delete_all_installation_versions(&self.conn, name, gpu_variant)
    }
    ...
    pub fn update_build_method(
        ...
        crate::db::queries::update_installation_source(&self.conn, name, gpu_variant, &source_json)
    }
    ...
    fn resolve_logical_id(&self, name: &str) -> Result<String> {
        Ok(crate::db::queries::get_installation_logical_id(&self.conn, name)?.unwrap_or_default())
    }
...
mod tests {
    use super::*;
    use crate::db::queries::InstallationRecord;
    ...
    #[test]
    fn test_available_installations_returns_options() {
        let manager = InstallationManager::open_in_memory().unwrap();
        ...
        let options = manager.available_installations().unwrap();
    ...
    #[test]
    fn test_available_installations_groups_by_variant() {
        let manager = InstallationManager::open_in_memory().unwrap();
