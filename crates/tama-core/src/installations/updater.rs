...
pub async fn check_latest_version(
    ...
    match backend {
        ...
        InstallationType::Custom => Err(anyhow!("Cannot check updates for custom backends")),
        InstallationType::Docker => Err(anyhow!("Cannot check updates for Docker backends")),
    }
}
...
pub async fn check_installation_updates(backend_info: &InstallationInfo) -> Result<UpdateCheck> {
...
pub fn supports_update_check(backend_type: &InstallationType) -> bool {
    matches!(
        backend_type,
...
mod tests {
    ...
    fn test_supports_update_check_llamacpp() {
        assert!(supports_update_check(&InstallationType::LlamaCpp));
    }

    #[test]
    fn test_supports_update_check_ikllama() {
        assert!(supports_update_check(&InstallationType::IkLlama));
    }

    #[test]
    fn test_supports_update_check_custom() {
        assert!(!supports_update_check(&InstallationType::Custom));
    }
    ...
    async fn test_check_latest_version_unsupported_backend() {
        let result = check_latest_version(&InstallationType::TtsKokoro, None, None).await;
        assert!(result.is_err());
        assert!(result
            .unwrap_err()
            .to_string()
