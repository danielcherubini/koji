...
fn test_build_full_args_non_unified_n_slots() {
    ...
    let server = h::sample_server(|s| {
        s.context_length = Some(8192);
        s.num_parallel = Some(4);
        s.kv_unified = false; // Non-unified (default)
    });
...
fn test_build_full_args_ctx_override_unified() {
    ...
    let server = h::sample_server(|s| {
        s.context_length = Some(8192); // Ignored because ctx_override takes priority
        s.num_parallel = Some(3);
        s.kv_unified = true; // Unified KV
    });
...
fn test_build_full_args_kv_unified_not_duplicated_when_in_user_args() {
    ...
    let server = h::sample_server(|s| {
        s.args = vec!["--kv-unified".to_string()]; // User manually added it
        s.context_length = Some(8192);
        s.num_parallel = Some(2);
        s.kv_unified = true; // Config also says unified
    });
