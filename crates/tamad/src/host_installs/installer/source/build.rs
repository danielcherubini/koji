...
use crate::host_installs::installer::InstallOptions;
...
pub(crate) fn build_cmake_args(
    ...
    if matches!(
        ...
        tama_core::installations::types::InstallationType::IkLlama
    ) {
...
mod tests {
    use super::*;
    use crate::host_installs::installer::source::detect;
    use std::path::PathBuf;
    use tama_core::installations::types::{InstallationSource, InstallationType};

    fn make_options(backend_type: InstallationType, gpu_variant: &str) -> InstallOptions {
        InstallOptions {
            backend_type,
    ...
    fn test_all_builds_include_rpath_origin() {
        let opts = make_options(InstallationType::LlamaCpp, "cpu");
    ...
    fn test_all_builds_use_static_libraries() {
        for variant in ["cpu", "cuda", "vulkan", "rocm", "metal"] {
            let opts = make_options(InstallationType::LlamaCpp, variant);
            let args = build_cmake_args(&opts, Path::new("/src"), Path::new("/build"), &[]);
            assert!(
    ...
    fn test_ik_llama_includes_iqk_fa_all_quants() {
        let opts = make_options(InstallationType::IkLlama, "cpu");
        let args = build_cmake_args(&opts, Path::new("/src"), Path::new("/build"), &[]);
        assert!(
    ...
    fn test_ik_llama_cuda_includes_both_flags() {
        let opts = make_options(InstallationType::IkLlama, "cuda");
        let args = build_cmake_args(&opts, Path::new("/src"), Path::new("/build"), &[]);
    ...
    fn test_rocm_emits_full_flag_set() {
        let opts = make_options(InstallationType::LlamaCpp, "rocm");
    ...
    fn test_non_rocm_never_emits_rocm_flags() {
        let opts = make_options(InstallationType::LlamaCpp, "cuda");
        let args = build_cmake_args(
            &opts,
            Path::new("/src"),
            Path::new("/build"),
    ...
    fn test_ik_llama_rocm_includes_both_iqk_and_rocwmma() {
        let opts = make_options(InstallationType::IkLlama, "rocm");
        let args = build_cmake_args(
            &opts,
            Path::new("/src"),
            Path::new("/build"),
