...
use crate::gpu::detect::detect_amdgpu_targets;
use crate::host_installs::installer::emit;
use crate::host_installs::installer::extract::find_backend_binary;
use crate::host_installs::installer::prebuilt::prepare_target_dir;
...
pub async fn install_from_source(
    ...
    // Check prerequisites
    let caps = tama_core::gpu::detect_build_prerequisites();
    ...
    if !caps.git_available
        && (matches!(options.backend_type, InstallationType::IkLlama) || commit.is_some())
    {
        return Err(anyhow!(
    ...
    let resolved_version = if version == "latest"
        && !matches!(options.backend_type, InstallationType::TtsKokoro)
    {
        match tama_core::installations::check_latest_version(&options.backend_type, None, None)
            .await
        {
            Ok(v) => {
...
async fn fetch_source(
    ...
    if matches!(backend_type, InstallationType::LlamaCpp)
        && commit.is_none()
...
async fn download_source_zip(
    ...
    let repo_match = match backend_type {
        InstallationType::LlamaCpp => "ggml-org/llama.cpp",
