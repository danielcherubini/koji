...
use std::sync::Arc;
use tama_core::backends::{BackendInfo, BackendType};
...
/// Seed a BackendManager with an installed llama_cpp backend (cpu variant).
fn seed_llama_cpp_backend(tmp_dir: &std::path::Path) {
    use tama_core::backends::BackendManager;
    let mgr = BackendManager::open(tmp_dir).unwrap();
...
/// Seed a BackendManager with a custom backend.
...
/// GET /tama/v1/backends on an empty registry returns 200 with backends=[],
...
/// GET /tama/v1/backends includes an installed llama_cpp entry.
#[tokio::test]
...
/// POST /tama/v1/backends/compaction without matching CSRF pair → 403;
...
async fn test_origin_enforcement_blocks_cross_origin_post() {
    ...
    let req = Request::builder()
        .method("POST")
        .uri("/tama/v1/backends/compaction")
        .header(
