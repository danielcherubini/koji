...
pub async fn register_backend(
    ...
    if let Err(resp) = crate::api::backends::reject_traversal(&req.name, "backend name") {
        return resp;
    }
    ...
    if let Err(resp) = crate::api::backends::reject_traversal(&req.version, "version") {
        return resp;
    }
    ...
    if is_docker {
        if let Err(e) = tama_core::backends::docker::docker_available().await {
            return error_response(
                StatusCode::BAD_REQUEST,
    ...
    // Parse backend_type string to BackendType enum
    let backend_type = match tama_core::backends::BackendType::from_str(&req.backend_type) {
        Ok(bt) => bt,
        Err(e) => {
            return error_response(
                StatusCode::BAD_REQUEST,
    ...
    let info = tama_core::backends::BackendInfo {
        name: req.name.clone(),
