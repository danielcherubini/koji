...
pub async fn rename_backend(
    State(state): State<Arc<ProxyState>>,
    Path(backend_name): Path<String>,
    Json(req): Json<RenameBackendRequest>,
) -> impl IntoResponse {
    if let Err(resp) = crate::api::backends::reject_traversal(&backend_name, "backend name") {
        return resp;
    }
...
fn validate_new_backend_name(name: &str) -> Result<String, String> {
    ...
    if crate::api::backends::is_path_traversal(&trimmed) {
        return Err(
