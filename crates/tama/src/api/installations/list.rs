...
pub async fn list_installations(
    ...
    let mut docker: Vec<InstallationCardDto> = Vec::new();
            ...
            for (type_, display_name, release_notes_url) in KNOWN_BACKENDS {
                ...
                if let Some(versions) = versions_opt {
                    ...
                    for (variant, variant_versions) in variant_groups {
                        ...
                        // Build version DTOs
                        let version_dtos: Vec<InstallationVersionDto> = sorted_versions
                            .iter()
                            .map(|info| InstallationVersionDto {
                        ...
                        let active_info = active_version.map(InstallationInfoDto::from);
                        ...
                        backends.push(InstallationCardDto {
            ...
            for name in &custom_names {
                ...
                if let Some(versions) = versions_opt {
                    ...
                    for (variant, variant_versions) in variant_groups {
                        ...
                        custom.push(InstallationCardDto {
                            r#type: bt.clone(),
            ...
            for name in &docker_names {
                ...
                if let Some(versions) = versions_opt {
                    ...
                    for (variant, variant_versions) in variant_groups {
                        ...
                        docker.push(InstallationCardDto {
                            r#type: bt.clone(),
                            backend_name: name.clone(),
                ...
                if let Some(versions) = versions_opt {
                    ...
                    for (variant, variant_versions) in variant_groups {
                        ...
                        let update_check = match active_version.as_ref() {
                            Some(info) => {
                                match tama_core::installations::check_installation_updates(info)
                                    .await
                                {
                    ...
                    backends.push(InstallationCardDto::default_uninstalled(
                        type_,
                        display_name,
                        *release_notes_url,
                        Vec::new(),
