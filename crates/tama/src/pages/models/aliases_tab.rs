...
#[cfg(test)]
mod tests {
    use super::validate_alias_name;

    /// Empty string returns an error.
    #[test]
    fn test_empty_string() {
        let result = validate_alias_name("");
        assert_eq!(result, Some("Alias name is required.".to_string()));
    }

    /// String longer than 128 characters returns an error.
    #[test]
    fn test_too_long() {
        ...
        assert_eq!(
            result,
            Some("Alias name must be 128 characters or fewer.".to_string())
        );
    }

    /// Exactly 128 characters is valid.
    #[test]
    fn test_max_length() {
        let name = "a".repeat(128);
        assert_eq!(validate_alias_name(&name), None);
    }

    /// Leading non-alphanumeric character returns an error.
    #[test]
    fn test_leading_special_char() {
        assert_eq!(
            validate_alias_name("_hello"),
            Some("Alias name must start with a letter or number.".to_string())
        );
        assert_eq!(
            validate_alias_name("-hello"),
            Some("Alias name must start with a letter or number.".to_string())
        );
        assert_eq!(
            validate_alias_name(".hello"),
    ...
    /// Illegal character in name returns an error.
    #[test]
    fn test_illegal_character() {
        assert!(validate_alias_name("hello world").is_some());
        assert!(validate_alias_name("hello@world").is_some());
        assert!(validate_alias_name("hello#world").is_some());
    }

    /// Valid names with allowed special characters pass.
    #[test]
    fn test_allowed_special_chars() {
        assert_eq!(validate_alias_name("my_alias"), None);
        assert_eq!(validate_alias_name("my-alias"), None);
        assert_eq!(validate_alias_name("my.alias"), None);
        assert_eq!(validate_alias_name("my/alias"), None);
    }

    /// Simple valid names pass.
    #[test]
    fn test_simple_valid() {
        assert_eq!(validate_alias_name("a"), None);
        assert_eq!(validate_alias_name("hello"), None);
        assert_eq!(validate_alias_name("MyModel123"), None);
        assert_eq!(validate_alias_name("1abc"), None);
    }
}
