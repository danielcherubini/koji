...
|------|-------------|-----|---------------------|
| [Gateway Dashboard Refactor & Telemetry](done/plan-192-gateway-dashboard-telemetry.md) | AI Gateway & Compute Fabric dashboard: gateway header control plane (status pill + cluster summary), Active Models section (ready/starting rows with live tok/s badge, Unload + benchmark-suite link), redesigned tamad host cards (VRAM double-unit fix, vendor-prefix GPU name cleaning, two-line GPU telemetry rows with `running_models` prop), pure inference telemetry grid (TG/PP 15m bar charts with ITL/prefill latency + cache & speculative card) replacing CPU/RAM | — ✅ COMPLETED (squash merge to main) | [`28f12ad4`](../../commit/28f12ad4) |
...
| [Benchmarks Frontend Refactor](done/plan-182-benchmarks-frontend-refactor.md) | Extract LlamaBenchForm, shared ModelQuantSelect/BackendSelect/submit helper, hoisted tab state, type dedup, dead code removal, utils tests (~500 lines dedup) | — ✅ COMPLETED (squash merge to main) | [`c70999fa`](../../commit/c70999fa) |
| [Typed API Responses](done/plan-174-typed-api-responses.md) | StatusResponse/ModelEntry/BackendModelEntry/OkResponse/ModelMutationResponse structs with golden shape tests, drift-guard round-trips, OpenAPI schema alignment (F19) | — ✅ COMPLETED (direct merge) | [`85249ff0`](../../commit/85249ff0) |
| [API Handler Boilerplate](done/plan-168-api-handler-boilerplate.md) | Eliminate repeated request-scaffolding — submit_benchmark_job (F13), resolve_model_record (F14), resolve_config_dir/open_repository kill CWD fallback (F15), spawn_blocking for all blocking SQLite (F20), dedupe backoff/mean_stddev/traversal guard/model editor (F37) | — ✅ COMPLETED (direct merge) | [`3ef68c64`](../../commit/3ef68c64) |
| [Pull Handler Tests](done/plan-166-pull-handler-tests.md) | 19 tests for pull HTTP surface + download/verify orchestration — validation (400/422/502), job GET/SSE, hash mismatch guard via wiremock HF endpoint; seam fix: `hf_api()` honors `HF_ENDPOINT` via `ApiBuilder::from_env()` | #167 ✅ COMPLETED | [`2c4cf784`](../../commit/2c4cf784), [`2a0529c6`](../../commit/2a0529c6), [`50bd85e3`](../../commit/50bd85e3), [`2be8709c`](../../commit/2be8709c), [`6077f9c1`](../../commit/6077f9c1), [`da0cb0bd`](../../commit/da0cb0bd) |
| [forward_request Tests](done/plan-165-forward-request-tests.md) | 8 direct tests for `forward_request` — dead-PID 502 + cleanup, circuit-breaker cooldown/trip, metrics counters, model-name rewrite via wiremock | — ✅ COMPLETED (direct merge) | [`8e5898ff`](../../commit/8e5898ff) |
| [OAuth2 Flow Tests](done/plan-164-oauth2-flow-tests.md) | ~16 tests for all 5 untested OAuth2 login-flow functions (handle_login, handle_login_callback, handle_logout, build_oauth2_client_from_config, fetch_userinfo) — CSRF cookie signing/validation, token exchange via wiremock, userinfo claim fallback, fail-soft degradation, logout cookie expiry | #166 ✅ COMPLETED | [`ccaf674a`](../../commit/ccaf674a) |
| [Backup Restore](done/plan-163-backup-restore.md) | Route `create_backup` at `GET /tama/v1/backup`; implement `run_restore()` with full additive merge (extract→validate→merge_model_cards→merge_database→merge_config→Config::to_db); fix `merge_database` to resolve `model_id` FK via JOIN with `model_configs` and complete all 32 data columns; rewrite `start_restore` with proper job lifecycle, in-memory config refresh, upload cleanup; 4 route-level tests (404, happy path, corrupt archive, tampered SHA); docs rewrite with correct field names | #165 ✅ COMPLETED | [`02a261bd`](../../commit/02a261bd), [`a53022e5`](../../commit/a53022e5), [`8a1ebfb7`](../../commit/8a1ebfb7), [`e5d197ba`](../../commit/e5d197ba), [`5b90954c`](../../commit/5b90954c) |
...
| [Sharded GGUF Pull](done/plan-179-sharded-gguf-pull.md) | Support pulling sharded GGUF models where multiple files in a subdirectory (e.g. `UD-Q4_K_XL/Laguna-S-2.1-UD-Q4_K_XL-00001-of-00003.gguf`) belong to a single quant — add `shards` field to QuantEntry, `group_sharded_quants` grouping function, `is_primary_shard` detection in verification, `is_safe_relative_path` validation + subdir creation, recursive `untracked_ggufs`, and frontend wizard shard handling | #164 ✅ COMPLETED | [`8609e514`](../../commit/8609e514), [`bbb833cf`](../../commit/bbb833cf), [`71baea8b`](../../commit/71baea8b), [`dee87639`](../../commit/dee87639), [`a2d13316`](../../commit/a2d13316), [`8b7dc63f`](../../commit/8b7dc63f), [`7d9ad132`](../../commit/7d9ad132), [`66ce7770`](../../commit/66ce7770), [`8660e8e7`](../../commit/8660e8e7), [`3e724bef`](../../commit/3e724bef), [`a697b70c`](../../commit/a697b70c), [`2a9ce03`](../../commit/2a9ce03), [`3ddbd6f`](../../commit/3ddbd6f), [`6ce57cf`](../../commit/6ce57cf) |
